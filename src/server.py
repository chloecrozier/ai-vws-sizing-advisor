...
# Ensure bleach is installed
try:
    import bleach
except ImportError:
    import subprocess
    import sys
    subprocess.check_call([sys.executable, "-m", "pip", "install", "bleach", "--break-system-packages"])
    import bleach
...
# Global dictionary to track running deployment processes
# Structure: {deployment_mode: {'vllm_process': subprocess.Popen}}
...
def get_static_vgpu_citations() -> Citations:
    """
    Provide static citations for vGPU documentation PDFs that exist in vgpu_docs/
    """
    import glob
    from pathlib import Path
    
    logger.info("🔍 Getting static vGPU citations...")
    
    # Use absolute path to vgpu_docs mounted in container
    vgpu_docs_path = Path("/workspace/vgpu_docs")
    logger.info(f"Looking for PDFs in: {vgpu_docs_path}")
    logger.info(f"Path exists: {vgpu_docs_path.exists()}")
    
    citations = []
    
    # Find all PDF files in vgpu_docs
    pdf_files = glob.glob(str(vgpu_docs_path / "*.pdf"))
    logger.info(f"Found {len(pdf_files)} PDF files: {pdf_files}")
    
    for pdf_path in sorted(pdf_files):
        file_name = os.path.basename(pdf_path)
        logger.info(f"Creating citation for: {file_name}")
        
        # Create citation metadata
        source_metadata = SourceMetadata(
            description=f"NVIDIA vGPU documentation: {file_name.replace('.pdf', '').replace('-', ' ')}"
        )
        
        # Create citation entry
        citation = SourceResult(
            content=f"📄 {file_name}\n\nOfficial NVIDIA vGPU documentation for sizing, configuration, and deployment guidance.",
            document_type="text",
            document_name=file_name,
            score=1.0,  # Static docs have high relevance
            metadata=source_metadata
        )
        citations.append(citation)
    
    logger.info(f"✅ Created {len(citations)} static citations")
    
    citations_obj = Citations(
        total_results=len(citations),
        results=citations
    )
    logger.info(f"Citations object: {citations_obj.model_dump()}")
    
    return citations_obj
                        ...
                        try:
                            ...
                            if first_chunk:
                                if contexts and len(contexts) > 0:
                                    logger.info(f"Using RAG citations from {len(contexts)} contexts")
                                    ...
                                    # Use static vGPU docs as fallback
                                    logger.info("No RAG contexts available, using static vGPU citations")
                                    chain_response.citations = get_static_vgpu_citations()
                                    logger.info(f"Set citations on chain_response: {chain_response.citations}")
                                first_chunk = False
                            ...
                            if first_chunk:
                                if contexts and len(contexts) > 0:
                                    logger.info(f"Using RAG citations from {len(contexts)} contexts (fallback path)")
                                    chain_response.citations = prepare_citations(
                                        retrieved_documents=contexts,
                                        collection_name=collection_name,
                                        enable_citations=prompt.enable_citations,
                                    ...
                                    # Use static vGPU docs as fallback
                                    logger.info("No RAG contexts available, using static vGPU citations (fallback path)")
                                    chain_response.citations = get_static_vgpu_citations()
                                first_chunk = False
                            logger.debug(response_choice)
                            yield "data: " + str(chain_response.model_dump_json()) + "\n\n"
...
async def generate_structured_answer(request: Request, prompt: Prompt) -> JSONResponse:
    ...
    try:
        ...
        # Add citations - use static vGPU docs if no RAG docs available
        if contexts and len(contexts) > 0:
            citations = prepare_citations(
                retrieved_documents=contexts,
                collection_name=collection_name,
            ...
            # Provide static vGPU documentation citations
            citations = get_static_vgpu_citations()
...
@app.get(
    ...
    responses={
        200: {
            "description": "GPU model detected",
            "content": {
                "application/json": {
                    "example": {
                        "gpu_model": "L40S",
...
async def detect_gpu(request: Request) -> JSONResponse:
    """Detect the GPU model on the current system using nvidia-smi."""
    try:
        import subprocess
        ...
        if result.returncode != 0:
            ...
            return JSONResponse(
                content={"error": "nvidia-smi not available or no GPU detected"},
                status_code=404
            )
        ...
        gpu_memory = int(float(gpu_info[1].strip())) if len(gpu_info) > 1 else 0
        
        # Normalize GPU name to match our profile names
        # Examples: "NVIDIA L40S" -> "L40S", "NVIDIA A40" -> "A40"
        gpu_model = gpu_name.upper()
        
        # Map common patterns to our GPU codes (order matters - check most specific first)
        if "RTX PRO 6000" in gpu_model or "BLACKWELL" in gpu_model or "BSE" in gpu_model:
            gpu_model = "BSE"
        elif "L40S" in gpu_model:  # Check L40S before L40 and L4
            gpu_model = "L40S"
        elif "L40" in gpu_model:  # Check L40 before L4
            gpu_model = "L40"
        elif "L4" in gpu_model:  # L4 must be checked last
            gpu_model = "L4"
        elif "A40" in gpu_model:
            gpu_model = "A40"
        else:
            # If we can't map it, return the raw name
            gpu_model = gpu_name
        
        logger.info(f"Detected GPU: {gpu_model} ({gpu_name}) with {gpu_memory}MB memory")
        ...
        return JSONResponse(
            content={"error": "GPU detection timed out"},
            status_code=500
        )
    except FileNotFoundError:
        logger.warning("nvidia-smi not found")
        return JSONResponse(
            content={"error": "nvidia-smi not installed"},
            status_code=404
        )
    except Exception as e:
        logger.error(f"Error detecting GPU: {str(e)}")
        return JSONResponse(
            content={"error": f"Failed to detect GPU: {str(e)}"},
            status_code=500
        )
...
async def test_configuration(request: Request, config_request: ApplyConfigurationRequest) -> StreamingResponse:
    """Test a vGPU configuration locally using Docker."""
    ...
    try:
        ...
        if deployment_mode == 'local':
            ...
            logger.info(f"Testing configuration locally")
            logger.info(f"Configuration details: {config_request.configuration}")
            
            # Test vLLM locally
            async def stream_local_test():
                """Stream local vLLM test progress as Server-Sent Events."""
                try:
                    logger.info("Starting stream_local_test generator")
                    print(">>> STARTING LOCAL VLLM DEPLOYMENT STREAM <<<")
                    async for progress in deploy_vllm_local(config_request):
                    ...
                    error_response = {
                        "status": "error",
                        "message": "Local vLLM test failed",
                        "error": str(e)
                    }
                    yield f"data: {json.dumps(error_response)}\n\n"
...
@app.get("/api/download-citation", tags=["Utility APIs"])
async def download_citation(source: str):
    """
    Download a vGPU documentation PDF
    """
    from pathlib import Path
    from fastapi.responses import FileResponse
    
    try:
        # Security: Only allow downloads from vgpu_docs directory
        vgpu_docs_path = Path("/workspace/vgpu_docs")
        file_name = os.path.basename(source)  # Extract filename only
        file_path = vgpu_docs_path / file_name
        
        # Verify file exists and is within vgpu_docs
        if not file_path.exists() or not file_path.is_relative_to(vgpu_docs_path):
            return JSONResponse(
                status_code=404,
        ...
        logger.error(f"Error downloading citation: {e}")
        return JSONResponse(
            status_code=500,
